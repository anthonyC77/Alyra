...
class App extends Component {
  ...
  runInit = async() => {
    ...
    const Voting = await contract.methods.getAddresses().call();
  ...
  Proposal = async() => {
     ...
     // // Interaction avec le smart contract pour ajouter un compte 
      await contract.methods.UsersProposalsRecord(userAddress, proposition).send({from: userAddress});
    
     // this.runInit();
   }
  
   Vote = async(idPropositionVote) => {
      ...
      // // // Interaction avec le smart contract pour ajouter un compte 
      await contract.methods.UsersVote(address, id).send({from: address});
  ...
  getProposals = async() => {    
    const { accounts, contract } = this.state;  
    // Interaction avec le smart contract pour décompter les votes 	
	this.state.proposalsId = await contract.methods.getProposalIds.call();
	 ...
	 return(
		<div style={{display: 'flex', justifyContent: 'center'}}>
		   <Card style={{ width: '50rem' }}>
			 <Card.Header><strong>Résultats du vote</strong></Card.Header>
			 <Card.Body>
  ...
  render() {
   ...
   if(isWhitelisted){
	   ...
	   if(this.state.stateVote == 2) return (
			...
			// proposition envoi mail pour être tenu au courant ?
	   );
						...
						<tbody>
						  {
							...
							<tr>
								<td>{a.name}</td>
								<td><Button onClick={() => this.Vote(a.id)} variant="dark" > Envoyer </Button></td>
							</tr>)
